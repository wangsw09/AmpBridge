...
cdef double _cproxLq_inv(double z, double t, double q):
...
def proxLq(x, t, q, tol=1e-8): # this seems faster than proxlLq
    if q == 1:
        return _cproxL1(x, t)
    elif q == 2:
        return _cproxL2(x, t)
...
def proxLq_dx(x, t, q, tol=1e-8): # this seems faster than proxlLq
    if q == 1:
...
def proxLq_dt(x, t, q, tol=1e-8): # this seems faster than proxlLq
    if q == 1:
...
def proxLq_inv(z, t, q): # this seems faster than proxlLq
    return _cproxLq_inv(z, t, q)
